...
@Tag(name = "MyPage", description = "일반 사용자 마이페이지 API - 모든 API JWT 필요")
...
public class MyPageController {
    ...
    @GetMapping
    @Operation(summary = "[인증] 내 정보 조회", description = "로그인된 사용자의 정보를 조회합니다.")
    public UserInfoResponse getMyInfo(@AuthenticationPrincipal Long userId) {
        return myPageFacade.getMyInfo(userId);
    }

    @GetMapping("/attendances")
    @Operation(summary = "[인증] 내 출석 현황 요약 조회", description = "내 출석 현황 요약을 조회합니다.")
    ...
    @Operation(summary = "[인증] 내 스케줄/출석 현황 조회", description = "전체 스케줄별 출석 현황을 조회합니다.")
